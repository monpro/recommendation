...
def give_recom_result(user_vec, item_vec, userid):
    ...
    for itemid in item_vec:
        ...
        res = np.dot(user_vector, item_vector)/(np.linalg.norm(user_vector)*np.linalg.norm(item_vector))
        record[itemid] = res
    for permutation in sorted(record.items(), key= operator.itemgetter(1), reverse=True)[:fix_num]:
        itemid = permutation[0]
        score = round(permutation[1], 3)
...
def ana_recom_result(train_data, userid, recom_list):
    ...
    print("recom result")
    for permutation in recom_list:
        print(item_info[permutation[0]])
