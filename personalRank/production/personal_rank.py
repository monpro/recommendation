...
def personal_rank(graph, root, alpha, iter_num, recom_num= 10):
    ...
    for iter_index in range(iter_num):
        ...
        rank = tmp_rank
    right_num = 0
    for combination in sorted(rank.iteritems(), key = operator.itemgetter(1), reverse=True):
...
def personal_rank_mat(graph, root, alpha, recom_num = 10):
    ...
    for combination in sorted(score_dict.iteritems(), key = operator.itemgetter(1), reverse= True)[:recom_num]:
